...
- `services/http-server.Dockerfile`: ARM-aware container build for the API and VROOM binary.
- `services/osrm.Dockerfile`: ARM-aware container build for OSRM.
- `compose/docker-compose.arm64.yml`: Compose service definition (API + OSRM).
- `env/http-server.arm64.env`: Default ARM runtime/build settings, including California map source.
